...
@section('content')
      ...
      <div class="container-fluid mt-4 card-container">
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                  ...
                  <li class="nav-item" role="presentation">
                        <a class="nav-link" id="clearance-tab" data-bs-toggle="tab" href="#clearance" role="tab"
            ...
            <div class="tab-content" id="myTabContent">
                  <div class="tab-pane fade show active" id="pending" role="tabpanel" aria-labelledby="pending-tab">
                        <div class="table-container">
                              <table class="table data-table" id="datatablesPending">
                                    ...
                                    <tbody>
                                          @foreach($clearanceList as $list)
                                                <tr class="open-clearance-approval" data-appno="{{ $list->cfAppNo }}"
                              ...
                              <div class="modal fade" id="clearanceModal" tabindex="-1">
                                    <div class="modal-dialog modal-xl">
                                          <div class="modal-content" id="clearanceModalContent">
                                          </div>
                                    </div>
                              </div>
                        </div>
                  ...
                  <div class="tab-pane fade" id="history" role="tabpanel" aria-labelledby="history-tab">
                        <div class="table-container">
                              <table class="table data-table" id="datatablesHistory">
                                    ...
                                    <tbody>
                                          @foreach($clearanceListHistory as $list)
                                                <tr class="open-clearance-history" data-appnohistory="{{ $list->cfAppNo }}"
                                                      data-cfid="{{ $list->cfID }}" style="cursor:pointer;">
                                                      <td>{{ $list->cfAppNo }}</td>
                                                      <td>{{ $list->Name }}</td>
                                                      <td>{{ $list->cfID }}</td>
                              ...
                              <div class="modal fade" id="clearanceModalHistory" tabindex="-1">
                                    <div class="modal-dialog modal-lg">
                                          <div class="modal-content" id="clearanceModalContentHistory">
                                          </div>
                                    </div>
                              </div>
                        </div>
                  ...
                  <!-- HR VIEW -->
                  <?php  if(session()->get('departmentCode') == "HR"){?>
                  <div class="tab-pane fade" id="clearance" role="tabpanel" aria-labelledby="hr-tab">
                        <div class="table-container">
      ...
      <!-- APPROVAL -->
      <script>
            $(document).on('click', '.open-clearance-approval', function () {

                  let appno = $(this).data('appno');
                  let cfid = $(this).data('cfid');

                  $.ajax({
                        url: "{{ route('clearance_approval_details') }}",
                        type: "POST",
                        data: {
                              appno: appno,
                              cfid: cfid,
                        ...
                        success: function (response) {
                              $('#clearanceModalContent').html(response);
      ...
      <!-- HISTORY  -->
      <script>
            $(document).on('click', '.open-clearance-history', function () {

                  let appnohistory = $(this).data('appnohistory');
                  let cfid = $(this).data('cfid');
                  $.ajax({
                        url: "{{ route('clearance_approval_details') }}",
                        type: "POST",
                        ...
                        success: function (response) {
                              $('#clearanceModalContentHistory').html(response);
                              $('#clearanceModalHistory').modal('show');
                        }
                  });
