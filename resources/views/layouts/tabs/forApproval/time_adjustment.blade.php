...
<div class="container-fluid mt-4 card-container">
      ...
      <div class="modal fade" id="taModal" tabindex="-1" aria-labelledby="taModalLabel"  aria-hidden="true"><!-- modal -->
            <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                              <h5 class="modal-title" id="taModalLabel">Time Adjustment Application Details</h5>
                              ...
                              <tbody>
                                    ...
                                    <tr>
                                          <td>
                                                ...
                                                @else
                                                <center>
                                                <input class="form-check-input" id="chkbox{{$num}}" type="checkbox"  value="{{$rows->enc_id}}" name="{{$rows->taAppNo}}" onclick="return CheckMe(1,this.id,'selectAll','datatablesPending')">
                                                </center>
                                                @endif 
                                          </td> 
                                          <td>{{$rows->taAppNo}}</td>
                                          ...
                                          <td>{{$rows->taID}}</td>
                                          <td>{{$rows->taName}}</td>
                                          <td>{{$rows->taCosCenter}}</td>
                                          <td>{{$rows->department}}</td>
                                          <td>{{$rows->taDate}}</td> 
                                          <td>{{$rows->taType}}</td>
                                          <td>{{$rows->taReason}}</td> 

                                          <td id="td{{$rows->taAppNo}}"> 
                                                <label class="fixed_lbl" id="lbl{{$rows->taAppNo}}" for="txt{{$rows->taAppNo}}"></label>
                                                <input type="text" id="txt{{$rows->taAppNo}}" class="form-control" maxlength="200"> 
                                          </td> 

                                          <td>
                                                <div class="btn-container"> 
                                                      <button id="{{$rows->taAppNo}}" class="btn btn-action btn-view" title="More Details" onclick="view_ta_details('{{$rows->enc_id}}',this.id,'{{$rows->enc_pay}}')"><i
                  ...
                  <div class="table-container">   
                        <div class="container">
                              <div class="row bg-light rounded p-2"> 
                                    ...
                                    <button class="btn btn-primary" id="btnFilter" onclick="Filter_History(1,this.id,2)">
...
<script>
      ...
      async function AppoverResponse(pint_mode,val,objID) {
            ...
            if (pint_mode==0){
                  ...
                  window.location.href='{{url("/time_adjust_approval")}}';
            }
            GlovalHTMLObjLoading(0,objID);  
            FadeOut();
      }
