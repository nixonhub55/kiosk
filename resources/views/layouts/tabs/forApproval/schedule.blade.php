...
<div class="container-fluid mt-4 card-container">
      ...
      <div class="modal fade" id="scModal" tabindex="-1" aria-labelledby="scModalLabel"  aria-hidden="true"><!-- modal -->
            <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                              <h5 class="modal-title" id="scModalLabel">Schedule Change Application Details</h5>
                              ...
                              <tbody>
                                    ...
                                    <tr>
                                          <td>
                                                ...
                                                @else
                                                <center>
                                                <input class="form-check-input" id="chkbox{{$num}}" type="checkbox"  value="{{$rows->enc_id}}" name="{{$rows->scAppNo}}" onclick="return CheckMe(1,this.id,'selectAll','datatablesPending')">
                                                </center>
                                                @endif 
                                          </td> 
                                          <td>{{$rows->scAppNo}}</td>
                                          ...
                                          <td>{{$rows->scID}}</td>
                                          <td>{{$rows->scName}}</td>
                                          <td>{{$rows->scCosCenter}}</td>
                                          <td>{{$rows->department}}</td>
                                          <td>{{$rows->scReqDate}}</td>  
                                          <td>{{$rows->scReason}}</td> 

                                          <td id="td{{$rows->scAppNo}}"> 
                                                <label class="fixed_lbl" id="lbl{{$rows->scAppNo}}" for="txt{{$rows->scAppNo}}"></label>
                                                <input type="text" id="txt{{$rows->scAppNo}}" class="form-control" maxlength="200"> 
                                          </td> 

                                          <td>
                                                <div class="btn-container"> 
                                                      <button id="{{$rows->scAppNo}}" class="btn btn-action btn-view" title="More Details" onclick="view_ta_details('{{$rows->enc_id}}',this.id,'{{$rows->enc_pay}}')"><i
                  ...
                  <div class="table-container">   
                        <div class="container">
                              <div class="row bg-light rounded p-2"> 
                                    ...
                                    <button class="btn btn-primary" id="btnFilter" onclick="Filter_History(1,this.id,6)">
...
<script>
      ...
      function view_ta_details(id,objID,enc_pay){
      ...
      async function AppoverResponse(pint_mode,val,objID) {
            ...
            if (pint_mode==0){
                ...
                //console.log(response);
                  window.location.href='{{url("/sc_approval")}}';
            }
            GlovalHTMLObjLoading(0,objID);  
            FadeOut();
      }
