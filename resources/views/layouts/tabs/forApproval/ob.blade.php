...
<div class="container-fluid mt-4 card-container">
      ...
      <div class="modal fade" id="obModal" tabindex="-1" aria-labelledby="obModalLabel"  aria-hidden="true"><!-- modal -->
            <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                              <h5 class="modal-title" id="obModalLabel">Official Business Application Details</h5>
                                    ...
                                    @foreach($list['rows'] as $rows)
                                          <tr>
                                                <td>
                                                      ...
                                                      @else
                                                      <center>
                                                      <input class="form-check-input" id="chkbox{{$num}}" type="checkbox" value="{{$rows->enc_id}}"  name="{{$rows->obAppNo}}" onclick="return CheckMe(1,this.id,'selectAll','datatablesPending')">
                                                      </center>
                                                      @endif 
                                                </td>
                                                <td>{{$rows->obAppNo}}</td>
                                                ...
                                                <td>{{$rows->obID}}</td>
                                                <td>{{$rows->obName}}</td>
                                                <td>{{$rows->obCosCenter}}</td>
                                                <td>{{$rows->department}}</td>
                                                <td>{{$rows->obType}}</td>
                                                <td>{{$rows->obReason}}</td> 

                                                <td id="td{{$rows->obAppNo}}"> 
                                                      <label class="fixed_lbl" id="lbl{{$rows->obAppNo}}" for="txt{{$rows->obAppNo}}"></label>
                                                      <input type="text" id="txt{{$rows->obAppNo}}" class="form-control" maxlength="200"> 
                                                </td> 

                                                <td>
                                                      <div class="btn-container"> 
                                                            <button id="{{$rows->obAppNo}}" class="btn btn-action btn-view" title="More Details" onclick="view_ob_details('{{$rows->enc_id}}',this.id,'{{$rows->enc_pay}}')"><i
                  ...
                  <div class="table-container">   
                        <div class="container">
                              <div class="row bg-light rounded p-2"> 
                                    ...
                                    <button class="btn btn-primary" id="btnFilter" onclick="Filter_History(1,this.id,3)">
...
<script>
      ...
      function view_ob_details(id,objID,enc_pay){
      ...
      async function AppoverResponse(pint_mode,val,objID) {
            ...
            if (pint_mode==0){
                  ...
                  window.location.href='{{url("/ob_approval")}}';
            }
            GlovalHTMLObjLoading(0,objID);  
            FadeOut();
      }
