            ...
            <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                              <h5 class="modal-title" id="overtimeModalLabel">Overtime Application Details</h5>
                        ...
                        <table class="table data-table" id="datatablesPending">
                              ...
                              <tr>
                                    <td>
                                          ...
                                          @else
                                           <center>
                                           <input class="form-check-input" id="chkbox{{$num}}" type="checkbox" value="{{$rows->enc_id}}" name="{{$rows->otAppNo}}" onclick="return CheckMe(1,this.id,'selectAll','datatablesPending')">
                                           </center>
                                          @endif
                                    ...
                                    <td>{{$rows->otID}}</td>
                                    <td>{{$rows->otName}}</td>
                                    <td>{{$rows->otAppDate}}</td>
                                    <td>{{$rows->otType}}</td>
                                    <td>{{$rows->otDate}}</td>
                                    <td>{{$rows->otTimeFrom}}</td>
                                    <td>{{$rows->otTimeTo}}</td>
                                    <td>{{$rows->otTimeTo}}</td>
                                    <td>{{$rows->otTotHours}}</td>
                                    <td>{{$rows->otReason}}</td> 

                                    <td id="td{{$rows->otAppNo}}"> 
                                          <label class="fixed_lbl" id="lbl{{$rows->otAppNo}}" for="txt{{$rows->otAppNo}}"></label>
                                          <input type="text" id="txt{{$rows->otAppNo}}" class="form-control" maxlength="200"> 
                                    </td> 

                                    <td>
                                          <div class="btn-container"> 
                                                <button id="{{$rows->otAppNo}}" class="btn btn-action btn-view" title="More Details" onclick="view_overtime_details('{{$rows->enc_id}}',this.id,'{{$rows->enc_pay}}')"><i
...
<script>
      ...
      var selectedID,this_mode,this_val;
      var chkbox_list = [];
      var chkbox_list_org = [];
      ...
      function view_overtime_details(id,objID,enc_pay){ 
      ...
      async function AppoverResponse(pint_mode,val,objID) {
            ...
            if (pint_mode==0){
                  ...
                  window.location.href='{{url("/overtime_approval")}}';
            }
            GlovalHTMLObjLoading(0,objID);  
            FadeOut();
      }
